...
class BaseGAN(object):
    ...
    def get_solution(self, XT_normalized):
        ...
        T = XT_normalized[:, 1][:, None] * self.T_std + self.T_mean
    ...
    def train(self, XTY_u, XT_r, XT_test):
        ...
        XT_u = XTY_u[:, 0:2]
        Y_test = self.get_solution(XT_test)
        ...
        for itr in range(self.num_itr):
            ...
            if (itr + 1) % 2000 == 0:
                ...
                rel_error = self.get_relative_error(XT_test, Y_test, l2, num_test)
                ...
                print(
                    "epoch_{}, rel_error is {:e} r2_loss is {:4f}, W loss is {:5f}; Time: {:4f}.\n".format(
                        itr + 1, rel_error, r2_loss, w_dis,
                        time.time() - time1))
                time1 = time.time()

        X_test = (np.linspace(-1, 1, 201) - self.X_mean) / self.X_std
        T_test = (np.linspace(0, 1, 5) - self.T_mean) / self.T_std
